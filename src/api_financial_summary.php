...
try {
    ...
    if ($pay_rate > 0) {
        ...
        while ($temp_pd > $current_date_time) {
            $temp_pd = $temp_pd->modify('-14 days');
        }
        while ($temp_pd < $current_date_time) {
            $temp_pd = $temp_pd->modify('+14 days');
        }
        ...
        if (!$true_next_payday_obj) {
            throw new Exception("Could not determine next payday with fixed reference.");
        }
        ...
        if ($current_date_time == $true_next_payday_obj) {
        ...
        if ($response["is_pay_day"]) {
            ...
            $hours_map = [];
            foreach ($logged_hours_list as $l) {
                $hours_map[$l['log_date']] = (float) $l['hours_worked'];
            }
            ...
            while ($loop_date <= $end_loop_check) {
                $ds = $loop_date->format('Y-m-d');
                $dow = (int) $loop_date->format('N');
